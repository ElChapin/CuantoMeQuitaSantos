...
.controller('View1Ctrl', function($scope) {
  ...
  var calculoSalario = function () {
    ...
    if (($scope.aporteAFC + $scope.aporteFVP) > $scope.salario * .3) {
        $scope.aporteAFC = 0;
        $scope.aporteFVP = 0;
    ...
    flujoEfectivoMensual -= $scope.aporteFVP + $scope.aporteAFC;
    
    //1.4:Deducciones
    $scope.interesesOCorreccionMonetariaPrestamosVivienda = 0;
    //medicina prepagada y pólizas de seguros
    $scope.pagosSalud = 0;						
    $scope.dependientes = false;
    ...
    var totalAportePension = IBC * .16 + $scope.aporteFVP;
    ...
    //3.1.3.1: Pagos salud
    var pagosSaludUVT = $scope.pagosSalud / UVT;
    var deduccionPagosSalud = (pagosSaludUVT > 16) ? 16 * UVT : $scope.pagosSalud;
    
    //3.1.3.2: Dependientes
    var deduccionDependientes = 0;

    if ($scope.dependientes) {
    ...
    var rentasExcentas = aporteObligatorioPension + $scope.aporteFSP + $scope.aporteFVP + $scope.aporteAFC;
    ...
    var deducciones = aporteObligatorioSalud + $scope.interesesOCorreccionMonetariaPrestamosVivienda + deduccionPagosSalud + deduccionDependientes;
    ...
    form.find('[name="aporteFSP"]').val(formatValueToCurency($scope.aporteFSP));
    form.find('[name="seguridadSocial"]').val(formatValueToCurency(aporteObligatorioSalud + aporteObligatorioPension + $scope.aporteFSP));
    ...
    form.find('[name="ahorroAnual"]').val(formatValueToCurency(cesantia + $scope.aporteAFC * 12));
  ...
  function calcularRetencion(baseGravable) {
